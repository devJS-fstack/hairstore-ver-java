...
if (accessToken != `null`) {
    (async () => {
        let { status, employee, role } = await checkToken(accessToken);
        if (status == 'success') {
            const idStore = location.href.split('?idStore=')[1];
            if (role == 3) {
                if (employee.store.idStore != idStore) {
                ...
                itemCustomer.forEach((item, index) => {
                    item.onclick = async (e) => {
                        if (!e.target.closest('.adm-actions__drop-down')) {
                            ...
                            const bookingsByPhone = bookings.filter(booking => booking.PhoneCustomer === item.getAttribute('data-customer'));
                            const bookingGroup = bookingsByPhone.reduce((acc, booking) => {
                                acc[booking.IDStaff] = (acc[booking.IDStaff] || 0) + 1;
                                return acc;
                            }, {});
                            var htmlBooked = ``;
                            var htmlService = ``;
                            const arrBookingGroup = Object.keys(bookingGroup);
                            if (arrBookingGroup.length > 0) {
                                console.log("bookingGroup: ", bookingGroup);
                                arrBookingGroup.forEach(key => {
                                    const staff = (rawStaffs ?? []).find(staff => staff.idStaff === key);
                                    htmlBooked += `<div class="sidebar-customer__most-appointments">
                        <div class="section-title">
                            <div class="adm-avatar size-40 mr-0 ml-0 avatar-employe_book"
                                style="background-image: url(${staff.pathImgStaff}); color: rgb(36, 112, 172);">
                            </div>
                            <div
                                class="adm-appointment-info-section-employee__text">
                                <div
                                    class="adm-appointment-info-section-employee__text__title">
                                    Lịch hẹn với (<span style="margin:0">${bookingGroup[key]}</span>)
                                </div>
                                <div
                                    class="adm-appointment-info-section-employee__text__name">
                                    <span>${staff.surName} ${staff.nameStaff}</span>
                                </div>
                            </div>
                        </div>
                    </div>`
                                ...
                                let payment = 0;
                                const bookingItems = bookingsByPhone.reduce((acc, booking) => {
                                    return [...acc, booking.bookItems].flat();
                                }, [])
                                const bookingItemGroup = bookingItems.reduce((acc, bookItem) => {
                                    if (!acc[bookItem.service.idService]) {
                                        acc[bookItem.service.idService] = {}
                                    }
                                    payment += bookItem.price;
                                    acc[bookItem.service.idService].count = (acc[bookItem.service.idService].count || 0) + 1;
                                    acc[bookItem.service.idService].nameService = bookItem.service.nameService;
                                    return acc;
                                }, {});
                                const arrBookItemGroup = Object.keys(bookingItemGroup)
                                arrBookItemGroup.forEach(key => {
                                    htmlService += `
                        <div class="adm-appointment-info-section-service services_customer mt-2">
                        <span>${bookingItemGroup[key].nameService}(${bookingItemGroup[key].count})</span>
                </div>
               `
                                })
            ...
            window.addEventListener('load', async () => {
                itemCustomer.forEach((item, index) => {
                    const bookingsFilter = bookings.filter(booking => booking.PhoneCustomer === item.getAttribute('data-customer'));
                    if (bookingsFilter.length) {
                        const bookingMaxByDate = bookingsFilter.reduce((max, booking) => 
                        (booking.DateBook > max.DateBook ? booking : max), bookingsFilter[0]); 
                        dateLastBook[index].textContent = moment(bookingMaxByDate.DateBook).format("yyyy-MM-DD");
                    }
                });
            })
            ...
            inputPhoneCus.oninput = async () => {
                ...
                if (inputPhoneCus.value.length == 10) {
                    const { status } = await checkDuplicatePhone_Customer(inputPhoneCus.value)
                    if (status == 'Found') {
            ...
            inputPhoneCus_edit.oninput = async () => {
                ...
                if (inputPhoneCus_edit.value != inputPhonePre_edit) {
                    if (inputPhoneCus_edit.value.length == 10) {
                        const { status } = await checkDuplicatePhone_Customer(inputPhoneCus_edit.value)
                        if (status == 'Found') {
            ...
            confirmAddCus.addEventListener('click', async (e) => {
                ...
                if (inputEmailCus.value == "") {
                    ...
                    if (validateEmail(inputEmailCus.value)) {
                        const { status } = await checkDuplicateEmail_Customer(inputEmailCus.value);
                        if (status === "Found") {
        ...
        <div class="adm-service__data">
            <div class="flex-center el-row is-align-middle"
                ...
                <div class="el-col el-col-24 el-col-sm-2"
                    ...
                    createCustomer(inputPhoneCus.value, inputNameCus.value, inputEmailCus.value);
            ...
            function clickEdit(editCustomer) {
                editCustomer.forEach((item, index) => {
                    item.onclick = async () => {
                        ...
                        successInputCategory(1, "")
                        successInputCategory(2, "");
                        const customer = customers.find(customer => customer.PhoneCustomer === item.getAttribute('data-customer'));
            ...
            confirm_editCus.addEventListener('click', async (e) => {
                ...
                if (inputEmailCus_edit.value == "") {
                    ...
                    if (inputEmailCus_edit.value != inputEmailPre_edit) {
                        if (validateEmail(inputEmailCus_edit.value)) {
                            const { status } = await checkDuplicateEmail_Customer(inputEmailCus_edit.value);
                            if (status === 'Found') {
                ...
                if (flag == 3) {
                    ...
                    const { status } = await editCustomerFunc(inputPhoneCus_edit.value, inputEmailCus_edit.value, inputNameCus_edit.value);
                    if (status === "success") {
                        await launch_toast("Vui lòng đợi trong giây lát...");
                        setTimeout(() => {
                            location.reload();
            ...
            function clickSetStatus(list, icons, spans) {
                list.forEach((item, index) => {
                    item.onclick = async () => {
                        console.log(spans[index].textContent.trim());
                        if (spans[index].textContent.trim() == 'Khóa tài khoản') {
                            ...
                            const { status } = await setStatusCustomer(item.getAttribute('data-customer'), 'No Active');
                            if (status === "success") {
                                await launch_toast("Vui lòng đợi trong giây lát...");
                                setTimeout(() => {
                                    location.reload();
                            ...
                            const { status } = await setStatusCustomer(item.getAttribute('data-customer'), 'Active');
                            if (status === "success") {
                                await launch_toast("Vui lòng đợi trong giây lát...");
                                setTimeout(() => {
                                    location.reload();
            ...
            async function setStatusCustomer(account, status) {
                return postData("customer/set-status", {
                    phone: account,
            ...
            async function editCustomerFunc(phone, email, name) {
                return postData("customer/edit", {
                    phone,
                    nameEmployee: name,
                    email,
                })
            ...
            async function createCustomer(phone, name, email) {
                return postData("customer/create", {
                    phone,
                    nameEmployee: name,
                    email,
                })
            ...
            async function checkDuplicateEmail_Customer(email) {
                return postData("user/check-duplicate", { email, referenceDuplicate: "EMAIL_CUSTOMER" });
            }

            async function checkDuplicatePhone_Customer(phone) {
                return postData("user/check-duplicate", { phone, referenceDuplicate: "PHONE_CUSTOMER" });
            }
